// src/features/(dashboard)/products/components/sections/ProductVariationsFields.tsx
"use client";

import { useEffect, useMemo, useState } from "react";
import { HelpCircle, Play, Plus, Search, UploadCloud, X } from "lucide-react";
...
/** Returns an Arabic error message, or null when the variations data is valid */
export function validateProductVariations(data: Step3FormData): string | null {
  if (!data.hasVariations) return null;

  if (data.attributes.length === 0) return "الرجاء اختيار سمات للمنتج";
  if (data.variations.length === 0) return "الرجاء إضافة قيمة واحدة على الأقل للاختلافات";

  const attributeIds = data.attributes.map((attr) => String(attr.id));
  let missingImage = false;
  let isValid = true;

  data.variations.forEach((row) => {
    attributeIds.forEach((attrId) => {
      if (!row.attributeValues[attrId]) isValid = false;
    });
    if (row.price <= 0) isValid = false;
    if (!row.imageFileName) missingImage = true;
  });

  if (missingImage) return "الرجاء رفع صورة لكل قيمة من قيم الاختلاف";
  if (!isValid) return "الرجاء إكمال جميع بيانات الاختلافات (القيم والسعر)";

  return null;
}

interface ProductVariationsFieldsProps {
  /** تُستخدم لجلب السمات الخاصة بفئة المنتج */
  categoryId?: number;
  initialData?: Step3FormData;
  onChange: (data: Step3FormData) => void;
}

/** Shared الاختلافات و الكميات fields for the product create/edit forms */
export function ProductVariationsFields({
  ...
  const buildSelectedOptionsFromVariations = (data?: Step3FormData): Record<number, number[]> => {
  ...
  const [hasVariations, setHasVariations] = useState<boolean>(initialData?.hasVariations || false);

  const [selectedAttributeIds, setSelectedAttributeIds] = useState<number[]>(() =>
    (initialData?.attributes || []).map((attr) => Number(attr.id))
  );

  const [variations, setVariations] = useState<VariationRow[]>(() => restoreVariations(initialData));
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setSelectedAttributeIds((initialData.attributes || []).map((attr) => Number(attr.id)));
  ...
  const selectedAttributesFull = useMemo(
    () => allAttributes.filter((attr) => selectedAttributeIds.includes(attr.id)),
    [allAttributes, selectedAttributeIds]
  );

  // إبلاغ الأب بالبيانات الحالية عند أي تغيير
  useEffect(() => {
    onChange({
      hasVariations,
  ...
  }, [hasVariations, selectedAttributesFull, variations]);
  ...
  const updateVariationRow = (id: string, field: keyof VariationRow, value: unknown) => {
    ...
    updateAttribute(
      {
        ...
        payload: { title: data.title, options: data.options },
      },
      {
        onSuccess: async () => {
    ...
    <>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <Tooltip
              ...
              content={
                "الاختلافات هي نسخ مختلفة من نفس المنتج تختلف في سمات معينة مثل الحجم أو اللون."
              }
            />
          </div>
        </div>
          ...
          {selectedAttributeIds.length > 0 ? (
                      ...
                      >
                          ...
                          return (
                            <div key={`${row.id}-${attr.id}`} className="flex justify-center w-full">
                              <ReusableDropdown
                                ...
                                onChange={(val) =>
                                  updateVariationAttributeValue(row.id, String(attr.id), val)
                                }
                        ...
                        <div className="relative">
                          <input
                            ...
                            value={row.price || ""}
                            onChange={(e) =>
                              updateVariationRow(row.id, "price", Number(e.target.value))
                            }
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <div className="flex items-center justify-center mb-6">
                <img src="/icons/dashboard/empty1.svg" alt="empty" className="w-40" />
              </div>
              <h3 className="text-lg font-bold mb-2">لم يتم إضافة اي سمات بعد!</h3>
            </div>
          )}
        </div>
      ) : null}
      ...
      <AttributeSelectionModal
        ...
        onConfirm={(ids, updatedAttrs, selectedOptsMap) => {
          ...
          if (updatedAttrs) {
            queryClient.setQueryData(["attributes", queryParams.toString()], (old: unknown) =>
              old ? { ...(old as object), data: updatedAttrs } : old
            );
          }
          ...
          if (ids.length > 0 && selectedOptsMap) {
            ...
            const attrOptions: { attrId: number; optIds: string[] }[] = attrs.map((a) => {
              ...
              const optIds =
                chosen && chosen.length > 0 ? chosen.map(String) : a.options.map((o) => String(o.id));
...
interface AttributeSelectionModalProps {
  ...
  onConfirm: (
    ids: number[],
    updatedAttributes?: api.Attribute[],
  ...
  const toggleOption = (attrId: number, optId: number) => {
    setSelectedOptions((prev) => {
      ...
      setLocalSelected((prevSelected) => {
        if (updated.length > 0) {
          return prevSelected.includes(attrId) ? prevSelected : [...prevSelected, attrId];
        }
    ...
    updateAttribute(
      ...
      },
      {
        onSuccess: async (res) => {
          ...
          const newOptId =
            res?.record?.options?.find((o: api.AttributeOption) => o.title === value)?.id ??
            Date.now();
          setLocalAttributes((prev) =>
            prev.map((a) =>
              a.id === attr.id
                ? { ...a, options: [...a.options, { id: newOptId, title: value, data: null }] }
                : a
            )
          );
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg p-0 overflow-hidden" dir="rtl">
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-6">
            {filteredAttributes.length > 0 ? (
              filteredAttributes.map((attr) => (
                <div key={attr.id}>
                  {/* عنوان السمة */}
                  <p className="text-sm font-semibold text-gray-800 mb-3 text-right">{attr.title}</p>
                  {/* الخيارات كـ chips */}
                  <div className="flex flex-wrap gap-2">
                    {attr.options.map((opt) => {
                      const isOptSelected = (selectedOptions[attr.id] || []).includes(opt.id);
                      return (
                        <button
                          key={opt.id}
                          type="button"
                    ...
                    {addingOptionFor === attr.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setNewOptionInputs((prev) => ({ ...prev, [attr.id]: e.target.value }))
                          }
                          onKeyDown={(e) => {
              ...
              <div className="flex flex-col items-center justify-center h-full gap-4 py-8">
                ...
                <div className="text-center text-gray-2 text-sm">لا توجد سمات مطابقة للبحث</div>
              </div>
            )}
          </div>
        </div>
        ...
        <DialogFooter className="p-4 border-t border-gray-100 bg-white shadow-2xl flex items-center justify-between sm:justify-between w-full">
          <div className="text-sm font-bold">{localSelected.length} سمات مختارة</div>
