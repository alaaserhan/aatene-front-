...
export function AddProductStep1({
    ...
    const { data: categoriesData, isLoading: isCategoriesLoading } = useGetCategories(
        useMemo(() => {
            ...
            params.set("page", String(categoriesPage));
        ...
        if (categoriesData?.data) {
            if (categoriesPage === 1) {
                ...
                setAllCategories((prev) => {
                    const newCats = categoriesData.data.filter((c) => !prev.some((p) => p.id === c.id));
                    return [...prev, ...newCats];
                });
            }
        }
    ...
    const handleLoadMoreCategories = () => {
        if (categoriesData && categoriesPage < Math.ceil(categoriesData.recordsFiltered / 10)) {
    ...
    const combinedFiles = useMemo(() => {
        return formData.cover ? [formData.cover, ...formData.gallery] : [...formData.gallery];
    ...
    const scrollToCategoryDropdown = () => {
        if (categoryDropdownRef.current) {
            categoryDropdownRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
        }
    };
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2" ref={categoryDropdownRef}>
                                        ...
                                        <ReusableDropdown
                                            ...
                                            onReachEnd={handleLoadMoreCategories}
                                            isLoadingMore={isCategoriesLoading && categoriesPage > 1}
                                            className="h-11"
                                        />
                                    </div>
                                        ...
                                        <Tooltip
                                            trigger={
                                                <div className="flex items-center gap-1 text-blue-4 cursor-pointer">
                                                    ...
                                                    <span className="text-xs font-medium">ماهو الوصف الموجز</span>
                                                </div>
                                            }
