...
export function ProductsPage({ storeId: propStoreId }: ProductsPageProps = {}) {
  ...
  const statusFromUrl = searchParams.get("status") as MerchantProductStatus | null;
  ...
  const [activeStatus, setActiveStatus] = useState<MerchantProductStatus | "all">(
    statusFromUrl && ["approved", "pending", "rejected"].includes(statusFromUrl)
      ? statusFromUrl
      : "approved"
  );
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-80px)]">
      ...
      {isMerchant && showGuide && <ProductGuide onClose={() => setShowGuide(false)} />}
