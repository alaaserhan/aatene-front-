// src/components/(admin)/analytics/CustomerLocationAnalytics.tsx
"use client";

import { PieChart, Pie, Cell, ResponsiveContainer } from "recharts";
import { ChevronsUpDown } from "lucide-react";

export function CustomerLocationAnalytics() {
  // البيانات الثابتة مع الألوان المحددة
  const data = [
    { name: "حيفا", value: 500, percentage: 40, color: "#22C55E", exploded: true }, // Green - Exploded
    { name: "يافا", value: 100, percentage: 10, color: "#3B82F6", exploded: false }, // Blue
    // { name: "عكا", value: 150, percentage: 15, color: "#60A5FA", exploded: false }, // Sky Blue
    { name: "القدس", value: 250, percentage: 25, color: "#374151", exploded: false }, // Dark Gray
    { name: "أريحا", value: 100, percentage: 10, color: "#F87171", exploded: false }, // Red/Pink
  ];
  ...
  const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, name, index }: any) => {
    ...
    // تدوير النص ليناسب زاوية الشريحة
    ...
    return (
      <text 
        ...
        fontSize="11"
        fontWeight="bold"
        transform={`rotate(${-midAngle}, ${x}, ${y})`} // تدوير بسيط مع الزاوية
      >
        {name}
      </text>
    );
            ...
            <tbody className="text-sm">
              {data.map((item) => (
                <tr key={item.name} className="group hover:bg-gray-50/50 transition-colors">
                  ...
                  {/* Percentage Badge (The Pill Shape) */}
